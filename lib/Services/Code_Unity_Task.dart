...
class UserCodeUnityTask_SheetsAPI {
  // Credentials
  static const _credentials = globals.credentials;
  // Spreadsheet ID
  static final _spreedsheetID = globals.spreadsheetID_Code_Unity_Task;
  // Sheet
  ...
  static Future<bool> updateId() async {
    if (_userSheet == null) return false;
    List<bool> check = [];
    final totalRowNumber = await UserCodeUnityTask_SheetsAPI.getRowCount();
    for (int i = 1; i <= totalRowNumber; i++) {
      check.add(await UserCodeUnityTask_SheetsAPI.updateCell(
        row: i,
        col: "id",
        value: i,
      ));
